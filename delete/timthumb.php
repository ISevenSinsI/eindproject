...
if(! defined('FILE_CACHE_TIME_BETWEEN_CLEANS'))	define ('FILE_CACHE_TIME_BETWEEN_CLEANS', 2620800);	// How often the cache is cleaned 

if(! defined('FILE_CACHE_MAX_FILE_AGE') ) 	define ('FILE_CACHE_MAX_FILE_AGE', 2620800);				// How old does a file have to be to be deleted from the cache
...
if(! defined('WATERMARK_OVERLAY_IMAGE') )	define( 'WATERMARK_OVERLAY_IMAGE', 'assets/img/watermark.png'); 
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$watermark = (bool) $this->param('wm', 1);
		...
		if ($watermark == 1) {
			...
			imagecopy( $canvas, $overlay_gd_image, $_GET["w"] - $overlay_width - 5, $new_height - $overlay_height - 5, 0, 0, $overlay_width, $overlay_height);
			imagealphablending($canvas, false );
			imagesavealpha($canvas , true);
		}
